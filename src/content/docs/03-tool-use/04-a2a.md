...
<div style="border-left:4px solid #f97316;padding:.8rem 1.2rem;margin:.8rem 0;background:rgba(255,255,255,0.03);border-radius:0 8px 8px 0;">
  <div style="font-weight:bold;color:#f97316;margin-bottom:.5rem;">Q: MCP 和 A2A 的核心区别是什么？能否只用其中一个？</div>
  <details>
    <summary style="cursor:pointer;color:#888;font-size:.9rem;">查看答案</summary>
    <div style="margin-top:.5rem;font-size:.9rem;">MCP 解决 Agent 与工具的连接（"我要用螺丝刀"），A2A 解决 Agent 与 Agent 的协作（"我请同事帮忙"）。它们解决不同层面的问题，通常一起使用：Agent A 通过 A2A 委托任务给 Agent B，Agent B 内部通过 MCP 调用工具完成任务。只用 MCP 无法实现跨 Agent 协作，只用 A2A 则 Agent 没有工具可用。</div>
  </details>
</div>
...
:::tip[与其他章节的关联]
A2A 是 Multi-Agent 架构的通信基础。在 [第 2 章：Multi-Agent 模式](/02-agent-patterns/04-multi-agent/) 中讨论的多 Agent 协作模式（如 Orchestrator-Worker、Debate），当这些 Agent 跨越组织和平台边界时，就需要 A2A 这样的标准协议来实现互操作。
:::
...
<div style="border-left:4px solid #60a5fa;padding:.8rem 1.2rem;margin:.8rem 0;background:rgba(255,255,255,0.03);border-radius:0 8px 8px 0;">
  <details>
    <summary style="font-weight:bold;color:#60a5fa;cursor:pointer;">自测题 1：A2A 和 MCP 最核心的区别是什么？</summary>
...
<div style="border-left:4px solid #60a5fa;padding:.8rem 1.2rem;margin:.8rem 0;background:rgba(255,255,255,0.03);border-radius:0 8px 8px 0;">
  <details>
    <summary style="font-weight:bold;color:#60a5fa;cursor:pointer;">自测题 2：Agent Card 的作用是什么？类比现实中的什么？</summary>
    <div style="margin-top:.8rem;font-size:.9rem;">
...
<div style="border-left:4px solid #60a5fa;padding:.8rem 1.2rem;margin:.8rem 0;background:rgba(255,255,255,0.03);border-radius:0 8px 8px 0;">
  <details>
    <summary style="font-weight:bold;color:#60a5fa;cursor:pointer;">自测题 3：一个完整的 Agent 协作场景中，MCP 和 A2A 如何配合？</summary>
    <div style="margin-top:.8rem;font-size:.9rem;">
