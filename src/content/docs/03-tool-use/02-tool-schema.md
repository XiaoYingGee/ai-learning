...
1. **为什么工具的 description 比 name 更重要？**
   - 答：LLM 主要依赖 description 来理解工具的用途、适用场景和使用时机，name 只是标识符。

2. **当系统有 50 个工具时，应该怎么处理？**
   - 答：使用分层路由（先分类再选工具）或动态加载（根据上下文只加载相关工具），避免一次性传入全部工具导致选择准确率下降。

3. **并行工具调用的前提条件是什么？**
   - 答：多个工具调用之间没有依赖关系，即后一个调用不需要前一个的结果。
