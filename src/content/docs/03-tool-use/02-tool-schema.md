...
<div style="border:2px solid #60a5fa;border-radius:12px;padding:1.2rem 1.5rem;max-width:500px;margin:1.5rem 0;">
  <div style="font-weight:bold;text-align:center;color:#60a5fa;margin-bottom:.8rem;">Tool Definition</div>
  <div style="font-size:.9rem;line-height:1.8;">
    <strong>name:</strong> <code>"search_products"</code> &larr; 工具名称<br/>
    <strong>description:</strong> <code>"在商品数据库中搜索商品，支持按名称、分类、价格范围过滤"</code> &larr; 工具描述（最关键）<br/>
    <strong>parameters:</strong> &larr; 参数 Schema<br/>
    &nbsp;&nbsp;&bull; <code>query</code> (string, required)<br/>
    &nbsp;&nbsp;&bull; <code>category</code> (enum, optional)<br/>
    &nbsp;&nbsp;&bull; <code>max_price</code> (number, optional)
  </div>
</div>
...
<div style="border:2px solid #60a5fa;border-radius:12px;padding:1.2rem 1.5rem;margin:1.5rem 0;">
  <div style="font-weight:bold;text-align:center;color:#60a5fa;margin-bottom:.8rem;">工具路由策略</div>
  <div style="font-size:.9rem;line-height:1.8;">
    <strong>策略 1：分层路由</strong><br/>
    Router &rarr; 先选分类 &rarr; 再选具体工具<br/>
    "用户管理" &rarr; [create_user, delete_user]<br/>
    "订单管理" &rarr; [create_order, query_order]<br/><br/>
    <strong>策略 2：动态加载</strong><br/>
    根据对话上下文只加载相关工具<br/>
    聊天关于订单 &rarr; 只加载订单相关工具<br/><br/>
    <strong>策略 3：工具描述中嵌入关键词</strong><br/>
    让 LLM 通过关键词匹配快速定位
  </div>
</div>
