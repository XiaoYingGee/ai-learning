...
:::tip[与其他章节的关联]
- Agent 和 Runner 实现了 [ch02 Agent 模式](/02-agent-patterns/01-what-is-agent/) 中的 Agentic Loop
- Handoff 概念与 [ch05-02 Claude Agent SDK](/05-frameworks/02-claude-agent-sdk/) 中的 Handoff 一致
- Guardrail 是 [ch02 Agent 安全](/02-agent-patterns/01-what-is-agent/) 中「护栏」概念的框架级实现
- Tool 定义同样基于 [ch03 工具调用](/03-tool-use/01-function-calling/) 中的 JSON Schema 规范
:::
...
<div class="card-quiz">
  <details>
    <summary>自测题 1：Guardrail 的 tripwire_triggered 设为 True 时会发生什么？</summary>
    <div class="answer">SDK 会立即中断 Agent 的执行流程并抛出异常，阻止潜在的不安全操作继续执行。</div>
  </details>
</div>

<div class="card-quiz">
  <details>
    <summary>自测题 2：Runner 的三种运行模式分别适用于什么场景？</summary>
    <div class="answer">run_sync 适合脚本和简单场景；run 适合异步 Web 服务；run_streamed 适合需要实时展示中间结果的聊天界面。</div>
  </details>
</div>

<div class="card-quiz">
  <details>
    <summary>自测题 3：Agent 的 Handoff 和直接调用另一个 Agent 的工具有什么区别？</summary>
    <div class="answer">Handoff 是控制权的完全转移，目标 Agent 接管后续所有交互；而调用工具只是临时借用能力，控制权仍在当前 Agent。</div>
  </details>
</div>
