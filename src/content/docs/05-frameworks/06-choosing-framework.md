...
:::tip[与其他章节的关联]
- 「不用框架」的选项对应 [ch02 Agent 模式](/02-agent-patterns/01-what-is-agent/) 中从零实现 Agent Loop 的思路
- 工具定义使用 MCP 标准以降低迁移成本，参见 [ch03 MCP 协议](/03-tool-use/03-mcp/)
...
<div class="card-quiz">
  <details>
    <summary>自测题 1：如果只需要一个简单的客服 Bot，需要用 LangChain 吗？</summary>
    <div class="answer">大概率不需要。简单场景直接用模型 SDK 的 Tool Use 功能即可，引入 LangChain 反而增加了不必要的复杂度和依赖。</div>
  </details>
</div>

<div class="card-quiz">
  <details>
    <summary>自测题 2：降低框架迁移成本的核心原则是什么？</summary>
    <div class="answer">将业务逻辑与框架解耦。具体做法包括：LLM 调用抽象为接口、工具定义使用 MCP 标准、Prompt 独立管理、避免深度依赖框架私有 API。</div>
  </details>
</div>

<div class="card-quiz">
  <details>
    <summary>自测题 3：在什么情况下应该考虑不用任何框架、自己从零实现？</summary>
    <div class="answer">当你需要极致的性能控制、极简的依赖、或者框架的抽象层成为调试瓶颈时。参考 Chapter 6 的从零实现方案。</div>
  </details>
</div>
