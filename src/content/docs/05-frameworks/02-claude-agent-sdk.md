...
:::tip[与其他章节的关联]
- Agent Loop 实现了 [ch02 Agent 模式](/02-agent-basics/02-agent-patterns/) 中的 ReAct 循环
- Tool Use 遵循 [ch03 工具调用](/03-tools-mcp/01-tool-calling/) 中介绍的 JSON Schema 定义规范
- MCP 集成对应 [ch03 MCP 协议](/03-tools-mcp/02-mcp-protocol/) 中的标准化工具协议
- Handoff 机制是 [ch02 多 Agent 编排](/02-agent-basics/02-agent-patterns/) 的具体实现
:::
...
<div style="border-left:4px solid #60a5fa;padding:.8rem 1.2rem;margin:.8rem 0;background:rgba(255,255,255,0.03);border-radius:0 8px 8px 0;">
  <details>
    <summary style="font-weight:bold;color:#60a5fa;cursor:pointer;">自测题 1：Claude Agent SDK 的 Agent Loop 在什么条件下会终止循环？</summary>
    <div style="margin-top:.8rem;font-size:.9rem;">当 Claude 的响应 stop_reason 不是 tool_use 时（即返回最终文本响应），循环终止。</div>
  </details>
</div>

<div style="border-left:4px solid #60a5fa;padding:.8rem 1.2rem;margin:.8rem 0;background:rgba(255,255,255,0.03);border-radius:0 8px 8px 0;">
  <details>
    <summary style="font-weight:bold;color:#60a5fa;cursor:pointer;">自测题 2：Handoff 和普通的工具调用有什么区别？</summary>
    <div style="margin-top:.8rem;font-size:.9rem;">工具调用执行完后控制权回到当前 Agent；Handoff 则是将整个对话控制权转移给另一个 Agent，由目标 Agent 继续处理后续交互。</div>
  </details>
</div>

<div style="border-left:4px solid #60a5fa;padding:.8rem 1.2rem;margin:.8rem 0;background:rgba(255,255,255,0.03);border-radius:0 8px 8px 0;">
  <details>
    <summary style="font-weight:bold;color:#60a5fa;cursor:pointer;">自测题 3：MCP 解决了什么核心问题？</summary>
    <div style="margin-top:.8rem;font-size:.9rem;">MCP 标准化了 LLM 与外部工具/数据源的连接协议，使得工具定义与具体 Agent 框架解耦，一个 MCP Server 可以被任意框架复用。</div>
  </details>
</div>
