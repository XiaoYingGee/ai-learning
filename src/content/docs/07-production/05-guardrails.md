...
用户输入
  ↓
[第 1 层] 输入格式校验（长度、编码、基本格式）
  ↓
[第 2 层] 恶意内容检测（Prompt Injection、敏感词）
  ↓
[第 3 层] LLM 系统提示（指令隔离、角色约束）
  ↓
[LLM 推理]
  ↓
[第 4 层] 输出内容检查（PII 脱敏、幻觉检测）
  ↓
[第 5 层] 格式合规校验（Schema 校验、长度限制）
  ↓
最终输出
```
...
<details>
<summary>1. 为什么简单的关键词过滤不足以防御 Prompt Injection？</summary>

攻击者可以用各种变体绕过关键词：拼写变化（igno re）、多语言混合、Unicode 同形字、Base64 编码等。需要结合模式匹配、LLM 分类器和多层防御。
</details>

<details>
<summary>2. 输出中的幻觉 URL 为什么需要特别处理？</summary>

LLM 可能生成看起来真实但实际不存在的 URL，用户点击后可能进入恶意网站或得到 404 错误，损害用户体验和信任。检测并标记未经验证的 URL 是必要的。
</details>

<details>
<summary>3. 分层防御中，哪一层最容易被绕过？为什么？</summary>

基于关键词的输入检测层最容易被绕过，因为攻击者可以轻松变换措辞。最可靠的是格式合规校验层，因为它基于确定性规则（如 JSON Schema），不存在语义模糊性。
</details>
