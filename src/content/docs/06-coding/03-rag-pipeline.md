...
<details>
<summary>1. chunk_overlap 的作用是什么？如果设为 0 会有什么问题？</summary>

chunk_overlap 让相邻块有一部分重叠内容，确保跨块边界的信息不会丢失。设为 0 时，恰好在块边界处的完整语句会被切断，检索时可能找不到完整的相关信息。
</details>

<details>
<summary>2. 为什么要批量处理 Embedding 而不是逐条处理？</summary>

API 调用有网络延迟开销。批量处理（一次发送 100 条）可以大幅减少 API 调用次数，降低延迟和成本。
</details>

<details>
<summary>3. 生成阶段的 System Prompt 中为什么强调"如果没有相关信息请如实说明"？</summary>
