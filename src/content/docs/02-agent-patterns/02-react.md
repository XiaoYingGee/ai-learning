...
flowchart LR
    subgraph CoT["只有推理（CoT）"]
        T1["思考"] --> T2["思考"] --> T3["思考"] --> Ans1["答案"]
    end
    subgraph ActionOnly["只有行动"]
        A1["行动"] --> A2["行动"] --> A3["行动"] --> Ans2["答案"]
    end
    subgraph ReAct["ReAct（推理+行动）"]
        Th1["思考"] --> Act1["行动"] --> Ob1["观察"] --> Th2["思考"] --> Act2["行动"] --> Ob2["观察"] --> Ans3["答案"]
    end
```
...
flowchart TD
    ...
    Observation -->|"未完成"| Thought
    Observation -->|"任务完成 / 达到最大步数"| Final["最终答案"]
```
