...
:::note[术语：Orchestrator]
Orchestrator（编排器/协调者）是多 Agent 系统中的中心控制节点，负责接收任务、分解子任务、分配给专业 Worker Agent、收集结果并整合最终输出。类似于软件工程中的"调度器"或管理学中的"项目经理"角色。
:::
...
:::note[术语：Blackboard Pattern]
黑板模式（Blackboard Pattern）源自经典 AI 研究，最初用于语音识别系统。其核心思想是：多个独立的"知识源"（这里是 Agent）共享一个公共数据结构（黑板），各自在上面读写信息。每个 Agent 只关注自己擅长的部分，当黑板上出现它能处理的数据时就介入。这种模式特别适合问题求解过程不确定、需要多种专业知识协同的场景。
:::
...
:::tip[与其他章节的关联]
A2A 和 MCP 协议在第 3 章有详细介绍。MCP 定义了 Agent 如何连接外部工具和数据源，A2A 定义了 Agent 之间如何互操作。理解这两个协议是构建生产级多 Agent 系统的基础。参见 [第 3 章：工具使用与 MCP/A2A](/03-tool-use/)。
:::
